...
class SpairBase(nn.Module):
    def __init__(self, image_shape):
        ...
        self.context_dim = (
            (cfg.N_LOOKBACK * 2 + 1) ** 2 // 2 * (4 + cfg.N_ATTRIBUTES + 1 + 1)
        )
        ...
        self.pixels_per_cell = tuple(
            int(i) for i in self.backbone.grid_cell_size
        )  # [pixel_x, pixel_y]
    ...
    def _compute_KL(self, z_pres, z_pres_prob):
        ...
        if (
            self.run_args.z_pres == "no_prior"
            or self.run_args.z_pres == "self_attention"
        ):
            return KL
        ...
        for h, w in itertools.product(range(H), range(W)):
            ...
            p_z_given_Cz = torch.clamp(
                count_support - count_so_far, min=0.0, max=(HW - i)
            ) / (HW - i)
            ...
            _obj_kl = prob * (safe_log(prob) - safe_log(p_z)) + (1 - prob) * (
                safe_log(1 - prob) - safe_log(1 - p_z)
            )
    ...
    def _build_obj_pres(self, obj_logits):
        ...
        obj_log_odds = torch.clamp(obj_logits, -10.0, 10.0)
    ...
    def _freeze_learning(self, *args):
        ...
        if len(ret) == 1:
            return ret[0]  # single element doesn't need unfolding
        return ret
    ...
    def _render(self, z_attr, z_where, z_depth_in, z_pres_in, x):
        ...
        obj_decoder_out = obj_decoder_out.view(
            -1, px, px, input_chan + 1
        )  # [Batch * n_cells, pixels_w, pixels_h, channels + alpha]
        ...
        if RunManager.run_args.z_pres == "self_attention":
            # QK dot V in softmax
            alpha_reshaped = alpha.view(-1, H * W, px * px).permute(
                0, 2, 1
            )  # Expand and move H*W to trailing dim
            attended_alpha = torch.bmm(alpha_reshaped, z_pres_in)
            reshaped_attended_alpha = (
                attended_alpha.permute(0, 2, 1,).contiguous().view(-1, px, px, 1)
        ...
        objects = torch.cat(
            [objects, alpha, importance], dim=-1
        )  # attach importance to RGBA, 5 channels to total

        # TODO debug output image, remove me later
        debug_tools.plot_prerender_components(
            objects, z_pres_flat, z_depth_flat, z_where, x
        )
        ...
        importance = (
            transformed_objects[:, :, input_chan + 1 : input_chan + 2, :, :] + 1e-9
        )
        ...
        # scale gradient
        weighted_grads_image = (
            img * importance
        )  # # + (1 - importance) * img.detach() # TODO Reinstate me!!!!!!!!
        # TODO DELETE ME
    ...
    def _build_loss(self, x, recon_x, kl):
        ...
        for name, z_kl in kl.items():
            ...
            self.writer.add_scalar(
                "losses/KL{}".format(name), kl_mean.detach(), self.global_step
            )
    ...
    def _debug_logging(self, z_where, z_attr, z_pres, z_depth):
        ...
        if (
            self.training_wheel == 1.0
        ):  # only start training when training wheel enables
            return
...
class Spair(SpairBase):
    def _compute_latent_vars(self, x, backbone_features):
        ...
        self.writer.add_scalar(
            "training_wheel", self.training_wheel.detach(), self.global_step
        )
        ...
        for h, w in itertools.product(range(H), range(W)):
            ...
            # --- z_depth ---
            layer_inp = torch.cat(
                [cell_feat, context, passthru_features, box, attr], dim=1
            )
    ...
    def _build_networks(self):
        ...
        self.box_network = build_MLP(
            inputsize, multiple_output=(n_localization_latent, n_passthrough_features)
        )
        ...
        self.object_encoder = build_MLP(
            n_inp_shape, n_attr_out, hidden_layers=[256, 128]
        )
        ...
        z_inp_shape = (
            ...
            + self.context_dim
            + cfg.N_BACKBONE_FEATURES
        )
        self.z_network = build_MLP(
            z_inp_shape, multiple_output=(2, n_passthrough_features)
        )  # For training pass through features
        ...
        input_chan = cfg.INPUT_IMAGE_SHAPE[0]
        decoded_dim = (
            obj_dim * obj_dim * (input_chan + 1)
        )  # [GrayScale + Alpha] or [RGB+A]
        self.object_decoder = build_MLP(
            cfg.N_ATTRIBUTES, decoded_dim, hidden_layers=[128, 256]
        )
    ...
    def _sample_z(self, mean, var, name, cell_coord):
        ...
        if name not in self.dist_param.keys():
            ...
            self.dist_param[name]["mean"] = torch.empty(
                self.batch_size, mean.shape[-1], H, W,
            ).to(self.device)
...
class ConvSpair(SpairBase):
    def _build_networks(self):
        ...
        # bounding box
        self.z_where_net = LatentConv(
            n_backbone_features,
            n_localization_latent * 2,
            additional_out_channels=n_passthrough_features,
        )
        if RunManager.run_args.use_z_where_decoder:
            self.NF_planar_x = fnn.SingleZPlanarNF2d(
                num_flows=10, q_z_nn_dim=n_backbone_features
            )
            self.NF_planar_y = fnn.SingleZPlanarNF2d(
                num_flows=10, q_z_nn_dim=n_backbone_features
            )
            self.NF_planar_h = fnn.SingleZPlanarNF2d(
                num_flows=10, q_z_nn_dim=n_backbone_features
            )
            self.NF_planar_w = fnn.SingleZPlanarNF2d(
                num_flows=10, q_z_nn_dim=n_backbone_features
            )

        z_depth_in = (
            4 + cfg.N_ATTRIBUTES + n_passthrough_features + cfg.N_BACKBONE_FEATURES
        )
        self.z_depth_net = LatentConv(
            z_depth_in, 2, additional_out_channels=n_passthrough_features
        )
        ...
        if RunManager.run_args.use_conv_z_attr:
            n_inp_shape = [
                ...
                obj_dim,
                obj_dim,
            ]  # flattening the 14 x 14 x 3 image
            self.object_encoder = ObjectConvEncoder(n_inp_shape, n_attr_out)
            self.object_decoder = ObjectConvDecoder(
                cfg.N_ATTRIBUTES, input_chan + 1
            )  # alpha channel
    ...
    def _compute_latent_vars(self, x: torch.Tensor, backbone_features):
        ...
        # with torch.cuda.stream(s):
        debug_tools.nan_hunter(
            "Before Main Loop", input_data=x, backbone_feature=backbone_features
        )
        # test_context = torch.empty(self.batch_size, 55, H, W).to(self.device)

        # --- z_where ---
        z_where, passthru_features, z_where_h = self.z_where_net(
            backbone_features, return_h=True
        )
        ...
        x_bundle = x_expanded.contiguous().view(
            batch_size * h * w,
            self.image_shape[0],
            self.image_shape[1],
            self.image_shape[2],
        ...
        # --- z_depth ---
        layer_inp = torch.cat(
            [backbone_features, passthru_features, local_bbox, attr], dim=1
        )
        ...
        debug_tools.nan_hunter(
            "Main Loop", normalized_box=bbox, attr=attr, depth=depth, presence=obj_pres
        )
    ...
    def _build_box(self, z_where, z_where_h):
        ...
        if RunManager.run_args.use_z_where_decoder:
            cy_logits, log_q_y = self._sample_z(
                cy_mean, cy_std, "cy_logit", save_dist=False, log_prob=True
            )
            cx_logits, log_q_x = self._sample_z(
                cx_mean, cx_std, "cx_logit", save_dist=False, log_prob=True
            )
            height_logits, log_q_h = self._sample_z(
                height_mean, height_std, "height_logit", save_dist=False, log_prob=True
            )
            width_logits, log_q_w = self._sample_z(
                width_mean, width_std, "width_logit", save_dist=False, log_prob=True
            )
            ...
            (
                ...
                height_logits,
                width_logits,
                self.normalizing_flow_loss,
            ) = self._freeze_learning(
                cx_logits, cy_logits, height_logits, width_logits, normalizing_flow_loss
            )
        else:
...
class ObjectConvDecoder(nn.Module):
    def __init__(self, input_size, output_channel):
        ...
        net["out"] = nn.ConvTranspose2d(
            in_channels=f, out_channels=output_channel, kernel_size=3
        )
        self.conv = Sequential(net)
