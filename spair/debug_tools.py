...
def benchmark(name="", print_benchmark=True):
    ...
    if print_benchmark:
        print("{} time: {:.4f} seconds".format(name, diff))
    return diff
...
def plot_cropped_input_images(cropped_input_images):
    ...
    input_imgs = cropped_input_images.permute(0, 4, 5, 2, 3, 1,).cpu().detach().numpy()
...
def _plot_image(title, data, gridspec, fig):
    ax = fig.add_subplot(gridspec)
    im = ax.imshow(data, vmin=0, vmax=1)  # Specific to the MNIST dataset
    # Disable axis
    ax.set_xticks([])
    ax.set_yticks([])
    if title:
...
def _plot_bounding_boxes(title, bbox, original_image, z_pres, gridspec, fig):
    ax = fig.add_subplot(gridspec)
    ax.imshow(original_image, vmin=0, vmax=1)
    # ptchs = []
    H, W, _ = bbox.shape
    for i in range(H):
        for j in range(W):
            ...
            border_color = (
                (1, 0, 0, pres) if pres > 0.5 else (0, 0, 1, pres)
            )  # red box if > 0.5, otherwise blue
...
def nan_hunter(hunter_name, **kwargs):
    ...
    telegram_yonk(
        "NaN Detected!! {}, step: {}".format(
            RunManager.run_name, RunManager.global_step
        )
    )
